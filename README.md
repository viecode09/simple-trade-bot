...
- **Market Analysis** (lebar penuh, di bawah Chart dan di atas Open Positions) — ditampilkan langsung tanpa klik, **2 kolom berdampingan**: **Current Situation** (tren per timeframe, gauge Trend Bias/RSI/Momentum, ringkasan) dan **Prediction** (BUY vs SELL bar, outlook/confidence, entry, TP, SL, R/R, visual level SL→Entry→TP, teks prediksi).
...
- `marginMode` — opsional (`cross`/`isolated`) untuk futures; memoverride `marginMode` di profile.
...
- **Auto TP/SL** (futures): setiap entry long/short otomatis memasang Take Profit **2×ATR** dan Stop Loss **1×ATR** reduce-only. Atur dengan `--tp-mult`, `--sl-mult`, `--atr-period` (set `0` untuk menonaktifkan).
...
Opsi: `--fast=7`, `--slow=25`, `--trend=99`, `--amount-type=quote|base`, `--dip`, `--dip-lookback=<n>` (default 3), `--rsi`, `--rsi-period=<n>` (default 14), `--rsi-oversold=<n>` (default 30), `--rsi-overbought=<n>` (default 70), `--tp-mult=<n>` (default 2), `--sl-mult=<n>` (default 1), `--atr-period=<n>` (default 14), `--dry-run`, `--watch`, `--interval` (detik).
